...
@notifications_bp.route("/timetable-notification", methods=["GET"])
def get_timetable_notification():
    """시간표 알림 설정 상태를 조회합니다."""
    user_id = session.get("student_id")
    if not user_id:
        return jsonify({"error": "로그인이 필요합니다."}), 401
    ...
    db = DatabaseManager()
    result = db.query(
        "SELECT timetable_noti_enabled FROM Students WHERE student_id = %(user_id)s",
        user_id=user_id
    ).result
    
    # 기본값을 FALSE로 설정
    enabled = bool(result[0][0]) if result else False
    return jsonify({"timetable_noti_enabled": enabled}), 200

@notifications_bp.route("/timetable-notification", methods=["POST"])
def toggle_timetable_notification():
    """시간표 알림 설정을 변경합니다."""
    user_id = session.get("student_id")
    if not user_id:
        return jsonify({"error": "로그인이 필요합니다."}), 401
    ...
    db = DatabaseManager()
    db.query(
        "UPDATE Students SET timetable_noti_enabled = %(enabled)s WHERE student_id = %(user_id)s",
        enabled=enabled,
        user_id=user_id
    )
    db.commit()

    return jsonify({"message": "시간표 알림 설정이 변경되었습니다.", "timetable_noti_enabled": enabled}), 200
